...
flags.DEFINE_integer('num_frame_per_clib', 15, 'Nummber of frames per clib')
...
FLAGS = flags.FLAGS
model_save_dir = '/media/senilab/DATA/Master/I3D-Tensorflow/experiments/ucf-101/models/flow_imagenet_101_20000_5_64_0.0001_decay'
...
def run_training():
    ...
    flow_pre_model_save_dir = "/media/senilab/DATA/Master/I3D-Tensorflow/checkpoints/flow_imagenet"
    ...
    train_writer = tf.summary.FileWriter('/media/senilab/DATA/Master/I3D-Tensorflow/visual_logs/train_flow_imagenet_101_30000_5_64_0.0001_decay', sess.graph)
    test_writer = tf.summary.FileWriter('/media/senilab/DATA/Master/I3D-Tensorflow/visual_logs/test_flow_imagenet_101_30000_5_64_0.0001_decay', sess.graph)
    start_time_all = time.time()
    for step in xrange(FLAGS.max_steps):
        start_time = time.time()
        rgb_train_images, flow_train_images, train_labels, _, _, _ = input_data.read_clip_and_label(
                      filename='/media/senilab/DATA/Master/I3D-Tensorflow/list/ucf_list/train.list',
        ...
        if step % 10 == 0 or (step + 1) == FLAGS.max_steps:
            ...
            rgb_val_images, flow_val_images, val_labels, _, _, _ = input_data.read_clip_and_label(
                            filename='/media/senilab/DATA/Master/I3D-Tensorflow/list/ucf_list/test.list',
                            batch_size=FLAGS.batch_size * gpu_num,
                            num_frames_per_clip=FLAGS.num_frame_per_clib,
                            crop_size=FLAGS.crop_size,
                            shuffle=True
            ...
            print("accuracy test: " + "{:.5f}".format(acc))
            test_writer.add_summary(summary, step)
        if (step+1) % 3000 == 0 or (step + 1) == FLAGS.max_steps:
            saver.save(sess, os.path.join(model_save_dir, 'i3d_ucf_model_scratch'), global_step=step)
    elapsed_time_all = time.time() - start_time_all
    print('elapsed time : %.3f sec' % (elapsed_time_all))
    print("done")
